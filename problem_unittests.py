...
def test_tokenize(token_lookup):
    ...
    # Check values type
    bad_value_type = [type(val) for val in token_dict.values()
...
def test_forward_back_prop(RNN, forward_back_prop, train_on_gpu):
    ...
    mock_decoder_optimizer = MagicMock(wraps=torch.optim.Adam(
        mock_decoder.parameters(), lr=learning_rate))
    ...
    with patch.object(torch.autograd, 'backward', wraps=torch.autograd.backward) as mock_autograd_backward:
        ...
        target = torch.LongTensor(
            np.random.randint(output_size, size=batch_size))
    ...
    assert (hidden_out[0][0] == hidden[0][0]).sum() == batch_size * \
        hidden_dim, 'Returned hidden state is the incorrect size.'
